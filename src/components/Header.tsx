...
// Define the shape of your notification object for TypeScript
...
// UPDATED: Using the Client ID you provided.
const clientId = "BG9-PhLnw4W3meQAkSNvpGG_rq8LeIjBjGFjMpu6CwG7XdUMcZKM3R31LA4dAYaz7-bNutzp-gpoYEgzW7EOgH4";

// Configuration using a dedicated Alchemy RPC URL
const chainConfig = {
  chainNamespace: CHAIN_NAMESPACES.EIP155,
  chainId: "0xaa36a7", // Sepolia Testnet
  rpcTarget: "https://eth-sepolia.g.alchemy.com/v2/Oc_ddJVUHkmB4wt6vK79k",
...
export default function Header({ onMenuClick, totalEarnings }: HeaderProps) {
  ...
  // Central function to handle successful login/session restoration
  const handleSuccessfulLogin = async (authInstance: Web3Auth) => {
    const user = await authInstance.getUserInfo();
    setUserInfo(user);
    setLoggedIn(true);
    ...
    if (user.email) {
      ...
      try {
        ...
        console.error("Error creating user (it might already exist):", error);
      }
    }
  };

  // Initializes Web3Auth inside the component's lifecycle
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        if (auth.connected) {
          await handleSuccessfulLogin(auth);
        }
      } catch (error) {
