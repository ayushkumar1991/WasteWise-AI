...
import { Trash2, MapPin, CheckCircle, Clock, Weight, Calendar, Search, Loader, Camera  } from 'lucide-react'
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const imageParts = [{
        inlineData: {
          data: base64Data,
          mimeType: 'image/jpeg', // This is a general default, the browser provides the correct one on upload
        },
      }];

      const prompt = `You are an expert in waste management. Analyze this image and confirm if it matches the reported waste. The reported waste is: Type: "${selectedTask.wasteType}", Quantity: "${selectedTask.amount}". Respond ONLY in a valid JSON format like this: {"wasteTypeMatch": true/false, "quantityMatch": true/false, "confidence": 0.9}. Do not include any other text or markdown formatting.`;
      ...
      try {
        // FIX: Clean the response text to robustly handle potential markdown formatting from the AI
